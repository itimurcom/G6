...
(function (global) {
  ...
  function typeToClass(t){ return t==='mi'?'type-mi':t==='nas'?'type-nas':t==='evt'?'type-evt':'type-other'; }
  function labelForType(t){ return t==='mi'?'ТЛГ: МИ':t==='nas'?'ТЛГ: НАС':t==='evt'?'Захід':'Інше'; }
  function findIndexById(arr,id){ for(var i=0;i<(arr||[]).length;i++){ if(arr[i] && arr[i].id===id) return i; } return -1; }
  function defaultTime(){ var d=new Date(); d.setMinutes(0,0,0); d.setHours(d.getHours()+1); return ('0'+d.getHours()).slice(-2)+':'+('0'+d.getMinutes()).slice(-2); }

  function pad2(n){ return ('0'+n).slice(-2); }
  function toISODate(d){ var y=d.getFullYear(); var m=('0'+(d.getMonth()+1)).slice(-2); var da=('0'+d.getDate()).slice(-2); return y+'-'+m+'-'+da; }
