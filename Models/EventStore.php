...
final class EventStore
{
    ...
    public function repairSummary(bool $apply = false): array
    {
        $before = $this->read();
        $eventsBefore = 0;
        foreach ($before as $d => $arr) { if (is_array($arr)) $eventsBefore += count($arr); }
        ...
        $idsBefore = 0; $idsAfter = 0;
        foreach ($before as $d=>$arr) { if (!is_array($arr)) continue; foreach ($arr as $e) { if (isset($e['id']) && is_string($e['id']) && $e['id']!=='') $idsBefore++; } }
        foreach ($after as $d=>$arr) { if (!is_array($arr)) continue; foreach ($arr as $e) { if (isset($e['id']) && is_string($e['id']) && $e['id']!=='') $idsAfter++; } }
        $assigned = max(0, $idsAfter - $idsBefore);

        $dupsRemoved = max(0, $eventsBefore - $eventsAfter);

        $daysTouched = 0;
        $allDays = array_unique(list(set(list(before.keys()) + list(after.keys())))) if False else None

        # Python placeholder removed; compute in PHP:
        // We'll recompute in PHP-style below.
        $daysTouched = 0;
